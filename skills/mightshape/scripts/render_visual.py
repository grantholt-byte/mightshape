...
PROVENANCE_STYLE = {
    ...
    "DESIGN_COUNCIL": ("◇", "MightShape", "#6457FF", "#EFEDFF"),
    ...
    if artifact_type == "AFFINITY_MAP":
        ...
        for cluster_index, cluster in enumerate(clusters):
            ...
            for note_index, note in enumerate(notes):
                ...
                _require_text(note.get("text"), f"{note_location}.text", 2000)
        ...
        for step_index, step in enumerate(steps):
            ...
            _require_text(step.get("detail"), f"{location}.detail", 2000)
...
def _svg_shell(artifact: dict[str, Any], width: int, height: int, body: str, description: str) -> str:
  ...
  <metadata>MightShape {_xml(artifact["artifact_type"])} {_xml(artifact["id"])}. Derived facilitation view; provenance remains attached to every record.</metadata>
  <defs>
...
def _affinity_note(note: dict[str, Any], x: int, y: int, width: int, outlier: bool = False) -> tuple[str, int]:
    text_width = max(29, min(54, int((width - 46) / 7)))
    source_width = max(31, min(58, int((width - 40) / 7)))
    text_lines = _lines(note["text"], text_width, 6)
    source_lines = _lines(_source_label(note), source_width, 2)
    ...
    text_svg, _ = _text_block(paper_x + 16, y + 72, note["text"], text_width, "note-text", 17, 6)
    source_svg, _ = _text_block(paper_x + 16, y + height - 18 - (len(source_lines) - 1) * 14, _source_label(note), source_width, "meta", 14, 2)
    outlier_mark = (
...
def _render_affinity_svg(artifact: dict[str, Any]) -> str:
    ...
    column_width = 872 if single_column else 276
    gap = 24
    margin = 24
    header_y = 170
    note_y = 347
    ...
    for column_index, (column_id, label, notes, is_outlier) in enumerate(columns):
        ...
        y_cursor = note_y
        if single_column:
            cards_per_row = min(3, max(1, len(notes)))
            card_gap = 18
            card_width = min(420, int((column_width - (cards_per_row - 1) * card_gap) / cards_per_row))
            row_width = cards_per_row * card_width + (cards_per_row - 1) * card_gap
            row_start = x + int((column_width - row_width) / 2)
            for row_start_index in range(0, len(notes), cards_per_row):
                row = notes[row_start_index : row_start_index + cards_per_row]
                row_heights = []
                for note in row:
                    text_width = max(29, min(54, int((card_width - 46) / 7)))
                    source_width = max(31, min(58, int((card_width - 40) / 7)))
                    text_lines = _lines(note["text"], text_width, 6)
                    source_lines = _lines(_source_label(note), source_width, 2)
                    row_heights.append(max(184, 92 + len(text_lines) * 17 + len(source_lines) * 14))
                row_height = max(row_heights)
                for offset, (note, note_height) in enumerate(zip(row, row_heights)):
                    note_x = row_start + offset * (card_width + card_gap)
                    note_metrics.append((note, note_x, y_cursor, card_width, note_height))
                y_cursor += row_height + 22
        else:
            for note in notes:
                ...
                note_metrics.append((note, x, y_cursor, column_width, note_height))
        ...
        header_text_width = 70 if single_column else 22
        detail_text_width = 106 if single_column else 38
        label_svg, _ = _text_block(x + 21, header_y + 55, label, header_text_width, "cluster", 17, 2)
        rendered_columns.append(label_svg)
        if not is_outlier:
            cluster = clusters[column_index]
            description = cluster.get("description", "")
            if description:
                description_svg, _ = _text_block(x + 21, header_y + 90, description, detail_text_width, "meta", 13, 2)
            ...
            rendered_columns.append(interpretation_svg)
        for note, note_x, y, note_width, _ in note_metrics:
            note_svg, height = _affinity_note(note, note_x, y, note_width, is_outlier)
...
<main>
  <header>
    <div class="eyebrow">◇ MightShape · Visual Workbench</div>
  ...
  <footer>Generated locally by MightShape Visual Workbench. No remote assets, scripts, or analytics.</footer>
</main>
</body>
</html>
'''
...
def _render_markdown(artifact: dict[str, Any], svg_name: str) -> str:
    ...
    lines.extend(["", "_Generated locally by MightShape Visual Workbench._", ""])
    return "\n".join(lines)
