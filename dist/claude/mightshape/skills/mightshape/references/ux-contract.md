...
- `◇ MIGHTSHAPE` product or Council activity
...
Substantive MightShape work defaults to `VISIBLE`; `QUICK_LOOK` and routine implementation default to `COMPACT`. An explicit request to see the exercise unfold selects `WORKSHOP`, presented to the user as `◇ OPEN STUDIO`. These modes control presentation, not methodological rigor.
