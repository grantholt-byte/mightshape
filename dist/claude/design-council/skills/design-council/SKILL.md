...
For an unfamiliar, regulated, safety-sensitive, or locally variable workflow, do a compact current-authoritative grounding pass before proposing product frames, even when no synthetic participant is requested. Separate the broad domain model from local facts that still require practitioner observation; if sources are unavailable, keep the model explicitly provisional.
