#!/usr/bin/env python3
"""Repository-level Design Council release validator.
...
def validate_package(root: str | Path) -> dict[str, Any]:
    ...
    before = len(errors)
    for relative in (".codex-plugin/plugin.json", "README.md", "LICENSE", "AGENTS.md", "skills/design-council/SKILL.md"):
    ...
    if manifest_path.exists():
        try:
            ...
            _check(interface.get("brandColor") == "#6457FF", "BRAND_COLOR", "expected Design Council brand color #6457FF", errors)
        except DesignCouncilError as exc:
            errors.append({"code": "MANIFEST_JSON", "message": str(exc)})
    report("plugin manifest", before)

    skill_root = root / "skills" / "design-council"
    before = len(errors)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate the complete Design Council package")
