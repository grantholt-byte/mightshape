...
The `EXCHANGE` participant-source provider, recruitment request schema, and credit-ledger schema
remain future-compatible architectural boundaries. In V1 the provider returns a structured
unavailable state. There is no participant marketplace, recruitment operation, verification
service, payment flow, credit balance, or purchasable entitlement behind it.

Keeping this boundary does not commit the project to charging later. It prevents a future human
research network—free, funded, partnered, or commercial—from requiring a rewrite of Inquiry Lab.

## When this decision may be revisited
...
Signals such as repeated requests for managed hosting, participant sourcing, team governance, or
enterprise assurance may inform that future conversation. They do not automatically activate a
paid plan. No pricing, paywall, or commercial experiment should be added merely because a schema
or provider boundary already exists.

## Marketplace policy fit

The free-product decision also keeps the adapters simple:

- the OpenAI plugin contains no digital-service promotion, plan comparison, upgrade link, credits,
  checkout, or advertising; and
- the Claude plugin is a useful open-source product rather than a promotional wrapper for an
  external service.

Recheck current platform rules before any future commercial change. The present release makes no
commercial offer and requires no monetization exception.
